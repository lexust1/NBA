...
def get_db_info(cur) -> Tuple[dict, pd.DataFrame]:
    ...
    for table_name in table_names:
        ...
        db_info[table_name] = pd.DataFrame(
            columns=[
                "ID", "Column Name", "Type", "NOTNULL", "DFLT_VALUE ", "PK"
            ],
            data=table_info
        ).set_index("ID")
...
def plot_mpl_bars(num_rows):
    ...
    plt.savefig(
        "figures/3.2 A num of rows in tables.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.close()
...
def plot_sns_bars(num_rows):
    """Creates the bar plot using seaborn."""
    # Set char, figure and grids.
    plt.figure(figsize=(12, 5))
    chart = sns.barplot(x="Table Name", y="A num of rows", data=num_rows)
...
def plot_plotly_bars(num_rows):
    """Creates the bar plot using plotly."""
    # Set the figure
    fig = go.Figure(data=go.Bar(
        x=num_rows["Table Name"],
        y=num_rows["A num of rows"],
        text=num_rows["A num of rows"],
        textposition="auto"))
    # Set the title and the axes names.
    fig.update_layout(
        title="A Number of Rows in Each Table",
        xaxis_title="Table Names",
        yaxis_title="A Number of Rows"
    )
    # Plot the chart with all settings.
    fig.show()
    ...
    for table_name in db_info:
        ...
        if table_name not in excluded_tables:       
            ...
            if not df.empty:
                ...
                profile.to_file(f"reports/03/profile_report_{table_name}.html")
        ...
        # If the table is too big and inside a list of excluded_table.
            ...
            if not df.empty:
                ...
                profile.to_widgets()
                # Save the output in the html-files.
                profile.to_file(f"reports/03/profile_report_{table_name}_{num_rows}_samples.html")
            # If the table (dataframe) is empty.    
            else: 
                empty_tables.append(table_name)    
    # Log the main data.
